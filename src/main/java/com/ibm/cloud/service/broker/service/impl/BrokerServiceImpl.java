...
public class BrokerServiceImpl implements BrokerService {
  ...
  /**
   * service or plan specific provisioning logic, add external provisioning call as needed, this saves new instance information on in memory db
   * @throws Exception
   */
  @Override
  public ResponseEntity<String> provision(
    ...
    CreateServiceInstanceRequest createServiceRequest = mapper.treeToValue(
      json,
    ...
    if (
      createServiceRequest.getContext() != null &&
      BrokerUtil.IBM_CLOUD.equals(
        createServiceRequest.getContext().getPlatform()
      )
    ) {
      Plan plan = CatalogUtil.getPlan(
        catalog,
        createServiceRequest.getServiceId(),
        createServiceRequest.getPlanId()
      );
      if (plan != null) {
        ServiceInstance serviceInstance = getServiceInstanceEntity(
        ...
        LOGGER.info(
          "Service Instance created: instanceId: {} status: {} planId: {}",
          instanceId,
        ...
        responseUrl
          ...
          .append(
            (displayName != null)
              ? displayName
              : catalog.getServiceDefinitions().get(0).getName()
          )
          .append(INSTANCE_ID)
          .append(instanceId);
        ...
        LOGGER.error(
          "Plan id:{} does not belong to this service: {}",
          createServiceRequest.getPlanId(),
          createServiceRequest.getServiceId()
        );
        return ResponseEntity
          .status(422)
          .body("Invalid plan id: " + createServiceRequest.getPlanId());
      }
    } else {
      LOGGER.error(
      ...
      response.put(
        "description",
        "Invalid platform: " + createServiceRequest.getContext().getPlatform()
      );
      response.put(
        BrokerUtil.ERROR,
        "Invalid platform: " + createServiceRequest.getContext().getPlatform()
  ...
  private String getServiceMetaDataByAttribute(String attribute) {
    ...
    if (service != null && service.getMetadata() != null) {
      if (
        service.getMetadata().containsKey(attribute) &&
        service.getMetadata().get(attribute) != null
      ) {
  ...
  /**
   * service or plan specific deprovisioning logic, add external deprovisioning call as needed, this removes instance information from in memory db
   * @throws Exception
   */
  @Override
  ...
  @Override
  public ResponseEntity<String> lastOperation(String instanceId, String iamId)
  ...
  /**
   * get catalog from RMC 
   * @throws Exception
   */
  @Override
  public ResponseEntity<String> importCatalog(MultipartFile file)
    ...
    ServiceDefinition service = mapper.treeToValue(
      catalogJson,
  ...
  public ResponseEntity<String> updateState(
    ...
    throws JsonProcessingException {
    ObjectMapper mapper = new ObjectMapper();
    UpdateStateRequest updateStateRequest = mapper.treeToValue(
      json,
  ...
  @Override
  public ResponseEntity<String> getState(String instanceId, String iamId)
    throws JsonProcessingException {
    ObjectMapper mapper = new ObjectMapper();
  ...
  private ServiceInstance getServiceInstanceEntity(
    ...
    instance.setName(
      (request.getContext() != null) ? request.getContext().getName() : null
    );
